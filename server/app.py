...
def zookeeper_by_id(id):
    ...
    if not animals:
        response_body =  "<h1>Zookeeper has no animals yet<h1>"
    else:
...
def enclosure_by_id(id):
    ...
    if not enclosure:
        response_body =  "<h1>ERROR 404: Enclosure not found<h1>"
        response = make_response(response_body, 404)
        return response
    ...
    response_body += f"<ul>Open to visitors?: {enclosure.id}</ul>"

    animals = [animal for animal in enclosure.animals]
    if not animals:
        response_body =  "<h1>Enclosure has no animals yet<h1>"
    else:
        for animal in animals:
            response_body += f"<ul>Animal name: {animal.name}</ul>"
    response = make_response(response_body, 200)
